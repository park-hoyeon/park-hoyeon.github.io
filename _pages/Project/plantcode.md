...
layout: category
permalink: /Project/plant/plantcode/
author_profile: true
sidebar_main: true
types: posts
taxonomy:
...
세션은 비밀번호와 같은 인증 정보를 쿠키에 저장하지 않고 서버에 저장한다. 대신 쿠키에는 사용자 식별자인 session-id를 저장합니다.  이처럼 세션도 쿠키를 이용하지만 추정 불가능한 session-id를 주고받기 때문에 보안상 안전하다. 따라서 노출되면 안 되는 중요한 사용자의 정보는 세션을 이용하기로 결정했다.
